...
static void list_journal_usage(void)
{
	puts("bcachefs list_journal - print contents of journal\n"
	     ...
	     "  -s, --seq-start=seq              First journal entry sequence to print\n"
	     "  -e, --seq-end=seq                Last journal entry sequence to print\n"
...
int cmd_list_journal(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "adn:s:e:BFMDHlLob:t:k:V:vh",
				  longopts, NULL)) != -1)
		switch (opt) {
		case 'a':
		...
		case 'n':
			...
			break;
		case 's':
			if (kstrtoull(optarg, 10, &seq_start))
				die("error parsing seq_start");
			opt_set(opts, read_entire_journal, true);
			break;
		case 'e':
			if (kstrtoull(optarg, 10, &seq_end))
				die("error parsing seq_end");
			opt_set(opts, read_entire_journal, true);
			break;
		case 'B':
			f.blacklisted = true;
