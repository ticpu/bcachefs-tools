...
static int bch2_dev_shrink(struct bch_fs *c, struct bch_dev *ca, u64 nbuckets,
			   struct printbuf *err)
{
	...
	if (ca->mi.freespace_initialized) {
		s64 delta = (s64)nbuckets - (s64)old_nbuckets;
		u64 v[3] = { delta, 0, 0 };
		...
		if (ret) {
			prt_printf(err, "accounting update error: %s\n", bch2_err_str(ret));
			return ret;
		}
	}
...
int bch2_dev_resize(struct bch_fs *c, struct bch_dev *ca, u64 nbuckets, struct printbuf *err)
{
	...
	if (nbuckets < ca->mi.nbuckets)
		return bch2_dev_shrink(c, ca, nbuckets, err);
