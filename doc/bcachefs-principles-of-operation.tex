...
stored \textbf{unencrypted}, even on an encrypted filesystem (see the Nocow
section below).
...
If the underlying storage is snapshotted externally (LVM, ZFS zvol, VM
snapshot) and the snapshot is mounted read-write, both instances share the
same master key and will reuse nonces, breaking encryption security.
bcachefs's own snapshots do not have this problem.
...
bitrot. On encrypted filesystems, nocow data is stored in plaintext. These
tradeoffs should be weighed carefully; for most workloads, the default COW
behavior with checksumming is the better choice.
...
per-device durability settings. See \hyperref[sec:devices]{Device management}
in subsystem details for the full treatment of caching modes (writeback,
writearound), durability counting, and how they interact with replication.
...
\texttt{bcachefs migrate} converts an existing filesystem (ext4, XFS, etc.)
to bcachefs in place, without reformatting or copying data to a separate
device. The original data is preserved and incorporated into the new bcachefs
filesystem. After migration, \texttt{bcachefs migrate-superblock} writes a
standard superblock so the filesystem can be mounted normally.
...
rolls back filesystem metadata to that point, discarding subsequent updates and
fixing up inconsistencies afterwards. This is a disaster recovery tool for
reverting damage from a bug in recovery that irreversibly corrupted or deleted
metadata, or (combined with \texttt{-o nochanges}) for attempting to recover
recently deleted files.

Journal rewind drops all leaf-level btree updates to non-allocator btrees at
or above the specified sequence number, then automatically triggers a full
fsck to rebuild allocation metadata.

\subsubsection{Usage}

Journal rewind is used in conjunction with \texttt{bcachefs list\_journal} to
identify the sequence number to rewind to. The
\texttt{journal\_transaction\_names} option (on by default) is required: it
controls whether \texttt{BCH\_JSET\_ENTRY\_overwrite} entries (the old values
being replaced) are stored in the journal, which is what rewind replays to undo
changes. It also records which function created each journal entry, making it
easier to identify which transactions caused a problem.
