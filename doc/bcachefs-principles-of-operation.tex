...
performance: metadata grows and IO becomes excessively fragmented.
...
immediately return
the extra-replica buckets to the write point for reuse, so the overhead of this
full data journalling approach is negligible in practice.
...
and are never read through EC reconstruction paths. If stripe creation fails
partway (e.g.\ a crash between writing parity and updating extents), the extra
replicas from the staging phase remain valid; the reconcile subsystem detects
the incomplete state and retries. Btree nodes are never placed in EC buckets;
this is explicitly checked and flagged as a filesystem inconsistency.
...
decrease replicas or change checksum settings on data they do not own.
...
set at format time to the log2 of the number of online CPUs if not
specified) partitions the inode number
space by CPU ID, giving each CPU its own allocation range. This eliminates
cross-CPU contention at the cost of non-sequential inode numbers.
