...
found. This restart-based approach avoids
deadlocks entirely and keeps worst-case latency bounded, at the cost of
...
but the corresponding journal entry was not. These sequences are blacklisted: recorded in the
superblock so that they are never replayed, and the bsets referencing them are
ignored until the btree node is next rewritten.
...
state. Once the allocator and journal
are initialized, the filesystem transitions to read-write and journal replay
runs - followed by the heavier validation and repair passes that need to write.
...
\begin{itemize}
	...
	\item The same algorithms are intended to be used for managing SMR
		...
		avoiding the translation layer in the drive. Bucket sizes can
		already be configured to match SMR zone sizes, but dedicated
		zoned device support is not yet implemented.
\end{itemize}
...
at 256 KB (controlled by \texttt{encoded\_extent\_max}). For most applications and usage scenarios this is an ideal trade off, but
...
is needed for NFS compatibility (NFSv2 and some NFSv3 implementations use
32-bit inode numbers), for 32-bit userspace that may truncate 64-bit inode
numbers, and for Wine/Proton/Steam, which run Windows applications that
assume 32-bit inode numbers internally. When combined with
\texttt{shard\_inode\_numbers} (which uses the CPU ID for the high bits of new
inodes to reduce allocator contention on large systems), the inode space is
partitioned so that each CPU gets a range within the 32-bit space.
...
can run fsck in userspace, in the kernel at mount time, or online on a mounted
filesystem, selecting the mode automatically or via command line options. The
in-kernel offline mode is useful when an older bcachefs-tools binary cannot
understand a newer filesystem format but the kernel module can. In all cases
the exact same fsck implementation runs - the recovery passes described above.
Running in userspace allows stopping with ctrl-c and prompting for fixes;
\texttt{bcachefs fsck -y} auto-fixes, \texttt{-n} runs read-only. When running
fsck at mount time via \texttt{-o fsck}, use \texttt{-o fix\_errors} to auto-fix.
...
serializes all CPUs. The \texttt{shard\_inode\_numbers} option partitions the
inode number space by CPU ID, giving each CPU its own allocation range. This
eliminates cross-CPU contention at the cost of non-sequential inode numbers.
Inode allocation cursors are persisted in the \texttt{logged\_ops} btree
(\texttt{KEY\_TYPE\_inode\_alloc\_cursor}) so that inode numbers are not reused
after a crash.
...
bcachefs uses 27 separate btrees for different data types, each identified by a
...
accounting counters). Flushing sorts the entire batch, deduplicates redundant
updates, then walks btree nodes in order for efficient bulk insertion; this
sort-merge-sweep is inherently single-threaded, making its efficiency critical
...
\begin{description}
	...
	\item \texttt{KEY\_TYPE\_error} \\
		Marks a region as containing unrecoverable errors.
